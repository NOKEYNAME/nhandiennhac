...
for version in bionic focal jammy lunar mantic noble; do
    ...
    cp -ra ../../ "${temp_dir}/nhandiennhac-2.3.4${version}"

    cd "${temp_dir}/nhandiennhac-2.3.4${version}"
    ...
    dput ppa:marin-m/songrec "../../nhandiennhac-2.3.4${version}_source.changes"

    cd "${ORIG_DIR}"

    rm -rf "${temp_dir}/nhandiennhac-2.3.4${version}"
